import * as olSource from 'ol/source';
import MVT from 'ol/format/MVT';
import * as TileGrid from 'ol/tilegrid'
import VectorTileSource from 'ol/source/VectorTile';
function metersVector() {
	const token =
		'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..ljZNFovWPka6r_ac9kg-mjUGMiW2gDgym-pUxnK2v1w';

	return new VectorTileSource({
		projection: 'EPSG:900913',
		attributions:
			'© <a href="https://www.mapbox.com/map-feedback/">Mapbox</a> ' +
			'© <a href="https://www.openstreetmap.org/copyright">' +
			'OpenStreetMap contributors</a>',
		format: new MVT(),
		tileGrid: TileGrid.createXYZ({ maxZoom: 22 }),
		url: 'https://tiles.mapinsights.portal.powercor.com.au/gwc/service/wmts?format=application/x-protobuf%3Btype=mapbox-vector&layer=map-insights:METERS&request=GetTile&service=WMTS&style=&tilecol={x}&tilematrix=EPSG:900913:{z}&tilematrixset=EPSG:900913&tilerow={y}&version=1.0.0&token=' +
			token,
	});
}

export default metersVector;
